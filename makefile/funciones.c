...
void deleteCurrentLine(int appData[], char windows[][appData[1]][appData[2]], int appCursors[][2]) {
	if (appCursors[appData[3]][0] != appData[1]) {

		int f, c;
		char supportMatrix[appData[1]][appData[2]];

		for (f = appCursors[appData[3]][0]; f < appData[1]-1; f++) {
			for (c = 0; c < appData[2]; c++) {
				supportMatrix[f][c] = windows[appData[3]][f+1][c];
			}
		}

		iniEmptyArray(appData[2], supportMatrix[appCursors[appData[3]][appData[1]]]);
	...
	} else if (appCursors[appData[3]][0] == appData[1]) {
...
void gotoLastLine(int appData[], int appCursors[][2]) {
	appCursors[appData[3]][0] = appData[1] - 1;
}
